...
export function ImageProcessor({
  ...
  const compressionFormat =
    effectiveOutputFormat === "png"
      ? "png"
      : effectiveOutputFormat === "jpg" || effectiveOutputFormat === "jpeg"
        ? "jpg"
        : null;
  ...
  const appendSelectedPaths = async (paths: string[]) => {
    ...
    const newPaths = filtered.filter(
      (path) => !images.some((item) => item.path === path),
    );
  ...
  const handleDrop = async (event: DragEvent<HTMLDivElement>) => {
    ...
    if (droppedPaths.length === 0) {
      setLocalError(
        "当前拖拽未提供可读文件路径，请改用“点击选择图片”或等待 Wails 文件拖拽事件接入。",
      );
      return;
    }

    await appendSelectedPaths(droppedPaths);
  };
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            <span
                              className={`rounded-full px-2 py-1 text-xs ${statusClassName(image.status)}`}
                            >
                              {statusLabel(image.status)}
                            </span>
                          </div>
                          <p className="mt-1 text-xs text-stone-500 dark:text-zinc-500">
                            原始 {formatBytes(image.size)} · {image.width} ×{" "}
                            {image.height}
                            {typeof image.outputSize === "number" &&
                              ` · 处理后 ${formatBytes(image.outputSize)}`}
                            {savingRate && ` · 节省 ${savingRate}`}
                            {typeof image.durationMs === "number" &&
                              ` · 用时 ${(image.durationMs / 1000).toFixed(1)}s`}
                          </p>
                            ...
                            <div className="mt-2 space-y-1">
                              <div className="h-1.5 overflow-hidden rounded-full bg-stone-200 dark:bg-zinc-800">
                                <div
                                  ...
                                  style={{
                                    width: `${stageProgress(image.processingStage)}%`,
                                  }}
                                />
                              </div>
          ...
          <div className="flex gap-2">
            {[
              ...
              [
                "jpg",
                "转 JPG",
                "JPG 属于有损压缩格式，通常更适合照片，文件体积通常更小，但不支持透明背景。",
              ],
              [
                "png",
                "转 PNG",
                "PNG 属于无损格式，适合透明背景、图标和需要保留细节的图片，但文件体积通常更大。",
              ],
            ...
            <div className="space-y-3">
              <div className="grid gap-2 md:grid-cols-3">
                ...
                <button
                  ...
                  className={`rounded-lg px-4 py-2 transition-colors ${
                    compressionType === "indexedColor" &&
                    compressionPreset === "256"
                      ? "bg-blue-600 text-white"
                      : "bg-stone-100 text-stone-700 hover:bg-stone-200 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700"
                  }`}
                >
